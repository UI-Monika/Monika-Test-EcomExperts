...
class QuantityInput extends HTMLElement {
  ...
  onInputChange(event) {
    ...
    const cartQuantity = document.querySelector(
      "[data-quantity-variant-id='45521462722843']"
    ).value;
...
class VariantSelects extends HTMLElement {
  ...
  onVariantChange() {
    ...
    this.thumbnailVariant();
    this.variantSelectBox();
  ...
  variantSelectBox() {
    document.querySelector('.customDropdownOuter .customSelected').innerText = this.currentVariant.option2;
    document.querySelector('.customDropdownOuter').classList.remove('active');


    if (document.querySelector('[value="Unselected"]').checked) {
      document.querySelector('product-form [type="submit"]').setAttribute('aria-disabled', "true");
    } else {
      document.querySelector('product-form [type="submit"]').removeAttribute('aria-disabled');
    }
  }

  thumbnailVariant() {
    const hideThumbnailsWithAlt = () => {
      const thumbnailElement = Array.from(
        document.querySelectorAll(".product__media-item[thumbnail-id]")
      );
      thumbnailElement.forEach((img) => {
        img.style.display = "none";
      });

      const currentImgAlt = this.currentVariant.option1;
      const thumbnailSelector = `[thumbnail-id ="${currentImgAlt.toLowerCase()}"]`;
      document
        .querySelectorAll(thumbnailSelector)
        .forEach((img) => (img.style.display = "block"));
    };

    hideThumbnailsWithAlt();
  }
